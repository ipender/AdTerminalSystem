...
/********** added by Pandeng ************************/
char *UVCCamera::getStreamCtrlInfo() {
    ENTER();
    if (mDeviceHandle == 0) {
        RETURN(NULL, char *);
    }
    ...
    UVCDiags params;
    if (mPreview) {
    		result = mPreview->probeUVCStreamCtrlInfo(&ctrl);
//    		str = params.getCurrentStream(&ctrl)
//    		LOGW("Frame baudrate Info: %s", str);
    		RETURN(params.getCurrentStream(&ctrl), char *);
    }
    RETURN(NULL, char *);
}
...
int UVCCamera::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format) {
	...
	int result = EXIT_FAILURE;
	if (mPreview) {
	    LOGW("set frame callback obj: %d", frame_callback_obj);
